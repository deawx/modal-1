(function(root, factory){
    ...
    Modal.prototype.template = function(){
        ...
        html += "<div id=\"ui-modal" + index + "\" class=\"ui-modal" + (options.skin ? (" " + options.skin) : "") + "\" style=\"z-index: " + options.zIndex + "; position: " + (options.fixed ? "fixed" : "absolute") + ";\">";
        if(options.title){
        ...
        if(options.cancel || options.ok){
            html += "<div class=\"ui-modal-foot\">";
            if(options.cancel){
                html += "<button type=\"button\" class=\"ui-modal-btn ui-modal-btn-cancel\">" + options.cancelText + "</button>";
            }
            if(options.ok){
                html += "<button type=\"button\" class=\"ui-modal-btn ui-modal-btn-ok\">" + options.okText + "</button>";
            }
            html += '</div>';
        }
        if(options.btnClose){
            html += "<button type=\"button\" class=\"ui-modal-btn-close\"></button>";
        }
        html += "</div>";
    ...
    Modal.prototype.handle = function(){
        ...
        // 注册取消按钮点击事件处理程序
        if(options.cancel){
            dom.on("click", ".ui-modal-btn-cancel", function(){
                var value = ready.isFunction(options.cancel) ? options.cancel(index, dom) : undefined;
        ...
        // 注册确定按钮点击事件处理程序
        if(options.ok){
            dom.on("click", ".ui-modal-btn-ok", function(){
                var value = ready.isFunction(options.ok) ? options.ok(index, dom) : undefined;
